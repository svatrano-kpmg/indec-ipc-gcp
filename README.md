...
1.  **Scheduler (OIDC) → Cloud Function (Launcher)**
    * `Cloud Scheduler` invoca a la CF `cf-indec-downloader-launcher`.
    * El payload JSON indica qué índice descargar (IPC/IPIM).
    * Esta función **no descarga el archivo**, sino que envía un Job a un cluster Dataproc existente (`sqlserver-cluster`).

2.  **Dataproc (Worker) → GCS Raw**
    * El cluster ejecuta el script `download_script.py`.
    * Descarga el archivo del INDEC, corrige el encoding (`latin-1` → `utf-8`) y lo guarda en el bucket Raw.
    * Al finalizar, publica un mensaje en el topic `raw.done`.

3.  **Pub/Sub (raw.done) → CF (Silver Transformer)**
...
4.  **Pub/Sub (curated.done) → CF (Gold SP Trigger)**
...
5.  **Pub/Sub (gold.done) → CF (Cuadro Tarifario)**
